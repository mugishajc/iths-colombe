...
class AppointmentController extends Controller
{
    ...
    public function deleteAppointment($id)
    {
        ...
        if(PatientAppointment::destroy($id)){
            return redirect()->back()->with('delete_appointment','Transfer deleted');
        }
    }
            ...
            if($appointment->save()){
                if($request->get('payment') != null || $request->get('payment') != ''){
                    ...
                    if($this->savePayment($payment,$request,$appointment->id)){
                        return response()->json(['Transfer Saved Successfully','Transfer has been saved successfully'],200);
                    }
                }
                return response()->json(['Transfer Saved Successfully','Transfer has been saved successfully'],200);
            }
